...
export class RolesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const ctx = tgContext.getContext<Context & { session?: { userEntity?: UserEntity } }>(); // Session type ni kengaytirish
    const chat = ctx.chat;
    ...
    if (!userEntity || !chat) {
      this.logger.warn('RolesGuard: Could not extract UserEntity from session or chat from Telegraf context.');
      throw new ForbiddenException('User or chat information is missing or user not authenticated.');
    }

    const userRoleInChat = await this.userManagementService.getUserRoleInChat(userEntity, chat.id);

    if (!userRoleInChat) {
      this.logger.warn(`RolesGuard: User ${userEntity.telegramId} has no role in chat ${chat.id}. Access denied.`);
      ...
      throw new ForbiddenException('You do not have a role assigned in this chat.');
    }
    ...
    if (!hasRequiredRole) {
      ...
      throw new ForbiddenException(`Your role (${userRoleInChat}) is not authorized to perform this action.`);
    }
    
    this.logger.verbose(`RolesGuard: User ${userEntity.telegramId} (role: ${userRoleInChat}) has required role for chat ${chat.id}. Access granted.`);
    return true;
  }
}
