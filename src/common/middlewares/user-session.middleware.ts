...
export class UserSessionMiddleware implements NestMiddleware {
  ...
  async use(ctx: TelegrafContextWithSession, next: (error?: any) => void) {
    if (!ctx.from) {
      this.logger.debug('No sender (ctx.from) in Telegraf context. Skipping user session setup.');
      return next();
    }

    if (!ctx.session) {
      this.logger.warn('ctx.session is not available. Ensure TelegrafSessionLocal middleware is applied before UserSessionMiddleware.');
    ...
    if (ctx.session.userEntity && ctx.session.userEntity instanceof UserEntity && ctx.session.userEntity.telegramId === ctx.from.id) {
    ...
    try {
      ...
      let user = await this.userManagementService.getUserByTelegramId(telegrafUser.id);
      if (!user) {
        ...
        this.logger.log(`New user ${user.telegramId} registered and added to session.`);
      } else {
        this.logger.debug(`User ${user.telegramId} found in DB and added/updated in session.`);
      }
      
      ctx.session.userEntity = user;

      if (telegrafChat && user) {
        await this.userManagementService.findOrCreateUserWithDefaultRoleInChat(user, telegrafChat);
        this.logger.debug(`UserChatRole ensured for user ${user.telegramId} in chat ${telegrafChat.id}`);
      }

    } catch (error) {
      this.logger.error('Error in UserSessionMiddleware while fetching or creating user:', error);
    }
