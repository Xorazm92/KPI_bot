...
export class SttProcessor {
  ...
  async handleTranscribeAudio(job: Job<SttJobData>): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Transcription received for job ${job.id}: ${transcription.substring(0, 50)}...`,
      );
      ...
      if (updatedLog && updatedLog.sttStatus === SttStatusEnum.COMPLETED) {
        this.logger.log(
          `[STT Job ${job.id}] STT result saved for messageLogId: ${messageLogId}. Adding LLM analysis job.`,
        );
        ...
        this.logger.warn(
          `[STT Job ${job.id}] STT result could not be saved or status not COMPLETED for messageLogId: ${messageLogId}. LLM job not added.`,
        );
      }
    } catch (error) {
      this.logger.error(
