...
export class ReportSubmissionUpdate {
  ...
  async onSubmitReportCommand(@Ctx() ctx: Context, @Message('text') messageText: string): Promise<void> {
    const telegramUser = ctx.from;
    const chat = ctx.chat; // Get chat from context

    if (!telegramUser || !chat) {
      this.logger.warn('Cannot identify user or chat from context for report submission');
      await ctx.reply('Sizni yoki chatni aniqlay olmadim. Iltimos, qaytadan urinib koʻring.');
      return;
    }
    ...
    if (!userEntity) {
      this.logger.log(`User with Telegram ID ${telegramUser.id} not found. Registering.`);
      ...
      this.logger.log(`User ${userEntity.id} (TG: ${telegramUser.id}) registered.`);
    }
    ...
    if (!user || !userChatRole) {
      this.logger.error(`Failed to get user or userChatRole for TGID: ${telegramUser.id} in chat ${chat.id}`);
    ...
    const currentMessage = ctx.message as TelegrafMessage.DocumentMessage | TelegrafMessage.TextMessage | undefined;
    const document = (currentMessage && 'document' in currentMessage) ? currentMessage.document : undefined;

    const parts = messageText.split(' ');
    const reportCode = parts.length > 1 ? parts[1] : undefined;

    if (!reportCode && !document) {
      await ctx.reply('Hisobot kodini kiriting yoki fayl yuklang. Masalan: /submit_report OYLIK_XARAJATLAR');
      return;
    }
    if (!reportCode && document){
        this.logger.warn('File uploaded with /submit_report command but without report code. This scenario needs clarification.');
        await ctx.reply('Fayl bilan hisobot kodini ham kiriting. Masalan: /submit_report OYLIK_XARAJATLAR [faylni biriktiring]');
        return;
    }

    this.logger.log(`Received /submit_report command with code '${reportCode}' from user ${user.telegramId}, document attached: ${!!document}`);
    ...
    try {
      const reportSubmission = await this.reportSubmissionService.handleReportSubmission(
        user,
      ...
      this.logger.error(`Error processing /submit_report command with code ${reportCode}: ${error.message}`, error.stack);
      await ctx.reply('Hisobotni qayta ishlashda kutilmagan server xatoligi.');
    }
  }

  @On('document')
  async onDocument(@Ctx() ctx: Context): Promise<void> { // Changed ctx type
    const telegramUser = ctx.from;
    const chat = ctx.chat; // Get chat from context
    ...
    const currentMessage = ctx.message as TelegrafMessage.DocumentMessage | undefined;
    if (!currentMessage || !('document' in currentMessage)) {
        this.logger.warn('Document message not found in context on document upload');
        return;
    }
    ...
    if (caption) {
        const match = caption.match(/#report\s+([\w-]+)/i) || caption.match(/report_code:\s*([\w-]+)/i);
        if (match && match[1]) {
            reportCodeFromCaption = match[1];
            this.logger.log(`Extracted report code '${reportCodeFromCaption}' from document caption.`);
        }
    }

    if (!reportCodeFromCaption) {
        this.logger.log(`Document received from user ${telegramUser.id} without explicit report code in command or caption. Document: ${currentMessage.document?.file_name}. Ignoring for now or applying default logic.`);
        return;
    }

    this.logger.log(`Document received with inferred report code '${reportCodeFromCaption}' from user ${user.telegramId} (Role: ${userChatRole.role})`);
    ...
    try {
      ...
      } else if (reportSubmission) { // Agar reportType bo'lmasa (bu holat bo'lmasligi kerak)
        await ctx.reply(
      ...
      this.logger.error(`Error processing document with inferred code ${reportCodeFromCaption}: ${error.message}`, error.stack);
      await ctx.reply('Faylni hisobot sifatida qayta ishlashda kutilmagan server xatoligi.');
    }
  }
}
